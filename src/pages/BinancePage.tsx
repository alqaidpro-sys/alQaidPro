...
const RATE = 54.4;
const NETWORKS = ["TRC20 (TRON)","BEP20 (BSC)","ERC20 (Ethereum)"];
const COINS = ["USDT","USDC","BNB","ETH","BTC"];

function FaqSection(){
  const [open,setOpen]=useState<number | null>(null);
  const faqs=[
    {q:"ما الشبكة الموصى بها؟",a:"TRC20 هي الأسرع والأقل تكلفةً من حيث الرسوم."},
    {q:"هل العنوان مهم جداً؟",a:"نعم، أي خطأ في عنوان المحفظة يؤدي لفقدان الأموال بشكل نهائي."},
    {q:"كم مدة التأكيد؟",a:"عادةً من 1 إلى 15 دقيقة حسب ازدحام الشبكة."},
  ];
  return(
...
function SuccessModal({coin,network,address,amount,egp,onClose}: any){
  return(
    <div className="modal-overlay" onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:24,width:"100%",maxWidth:380,animation:"slideDown .35s ease both"}}>
        <div style={{textAlign:"center",marginBottom:18}}>
          ...
          <div style={{fontSize:12,color:C.sub,marginTop:6,lineHeight:1.8}}>سيتم إرسال الكريبتو بعد التحقق من الطلب</div>
        </div>
        <div style={{background:C.cardAlt,borderRadius:12,padding:"14px",marginBottom:16}}>
          {[["العملة",coin,C.text],["الشبكة",network,C.sub],["المبلغ",`${amount} ${coin}`,C.yellow],["العنوان",address.slice(0,16)+"...", "#4F8EF7"],["المخصوم",`£ ${egp}`,C.red]].map(([l,v,c],i,a)=>(
...
export default function BinancePage({balance=5000,setBalance,onBack,onAddToCart}: any){
  ...
  const handleAddToCartInternal = () => {
    ...
    if(num<=0){setError("يرجى إدخال مبلغ صحيح");return;}
    if(onAddToCart) {
      onAddToCart({ name: `تحويل ${coin}`, icon: "₿", total: egp, fields: { "العنوان": address, "المبلغ": `${num} ${coin}`, "الشبكة": network } });
      setShowAdded(true);
    }
  };
  ...
  return(
    <>
      <style>{css}</style>
      {showModal&&<SuccessModal coin={coin} network={network} address={address} amount={num} egp={egp} onClose={()=>setShowModal(false)}/>}
      <div dir="rtl" style={{fontFamily:"'Cairo',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",paddingBottom:40}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 16px 16px"}}>
          ...
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:16,fontWeight:900,color:C.text}}>Binance & Crypto USDT</div>
            <div style={{fontSize:12,fontWeight:700,color:C.sub}}>تحويل كريبتو</div>
          </div>
        ...
        <div style={{padding:"0 14px",display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 16px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:12,color:C.sub,display:"flex",alignItems:"center",gap:5}}><span style={{color:C.yellow}}>⚡</span><span>تحويل سريع</span></div>
              </div>
              <div style={{textAlign:"left"}}>
                <div style={{fontSize:11,color:C.sub2,marginBottom:5}}>السعر / 1 USDT</div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.borderAlt}`,borderRadius:14,padding:"13px 16px"}}>
            <p style={{fontSize:13,color:C.sub,lineHeight:1.9}}>أرسل USDT أو أي عملة رقمية لأي محفظة كريبتو. تأكد من اختيار الشبكة الصحيحة.</p>
          </div>

          {/* COIN */}
          <div className="fadeUp">
            <label style={{fontSize:12,color:C.sub,fontWeight:700,display:"block",marginBottom:8}}>العملة الرقمية</label>
            <select className="inp" value={coin} onChange={e=>setCoin(e.target.value)} style={selectStyle as any}>
          ...
          <div className="fadeUp" style={{animationDelay:".05s"}}>
            ...
            <select className="inp" value={network} onChange={e=>setNetwork(e.target.value)} style={selectStyle as any}>
              {NETWORKS.map(n=><option key={n} value={n}>{n}</option>)}
            </select>
            <div style={{marginTop:8,background:"rgba(212,160,23,.06)",border:`1px solid rgba(212,160,23,.2)`,borderRadius:10,padding:"9px 13px"}}>
              <span style={{fontSize:11,color:C.yellow}}>⚠️ تأكد أن الشبكة المختارة تطابق محفظة المستلم</span>
            </div>
          </div>

          {/* ADDRESS */}
          <div className="fadeUp" style={{animationDelay:".08s"}}>
            <label style={{fontSize:12,color:C.sub,fontWeight:700,display:"block",marginBottom:8}}>عنوان المحفظة</label>
            <input className="inp" value={address} onChange={e=>setAddress(e.target.value)} placeholder="أدخل عنوان المحفظة" dir="ltr" style={{...inputStyle,fontSize:12,letterSpacing:.5} as any}/>
          </div>

          {/* AMOUNT */}
          <div className="fadeUp" style={{animationDelay:".1s"}}>
            <label style={{fontSize:12,color:C.sub,fontWeight:700,display:"block",marginBottom:8}}>المبلغ ({coin})</label>
            <div style={{position:"relative"}}>
              <input className="inp" value={amount} onChange={e=>setAmount(e.target.value.replace(/[^0-9.]/g,""))} placeholder="مثال: 10" dir="ltr" style={{...inputStyle,paddingLeft:70} as any}/>
              <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:11,color:C.sub2,fontWeight:700,pointerEvents:"none"}}>{coin}</span>
            </div>
          </div>

          {/* EGP */}
          <div className="fadeUp" style={{animationDelay:".13s"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <label style={{fontSize:12,color:C.sub,fontWeight:700}}>المبلغ بالجنيه المصري</label>
              <span style={{fontSize:10,color:C.sub2}}>1 USDT = £{RATE} · تلقائي</span>
            </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
            <button className="btn" onClick={handleBuy} disabled={loading}
              ...
              {loading?<span style={{display:"inline-flex",alignItems:"center",gap:10}}><span style={{width:18,height:18,border:"2px solid rgba(0,0,0,.2)",borderTopColor:"#1a0f00",borderRadius:"50%",animation:"spin 1s linear infinite",display:"inline-block"}}/>جاري...</span>:"إرسال الكريبتو ⚡"}
            </button>
