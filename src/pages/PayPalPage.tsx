...
const RATE = 54.4;
const PLATFORMS = ["PayPal","Payeer","Skrill"];
const CURRENCIES = ["USD","EUR","GBP"];

function FaqSection() {
  const [open, setOpen] = useState<number | null>(null);
  const faqs = [
    {q:"كم مدة التحويل؟", a:"يتم التنفيذ خلال 5–30 دقيقة بعد مراجعة الطلب."},
    {q:"هل البريد الإلكتروني ضروري؟", a:"نعم، البريد هو هويتك في محافظ PayPal و Payeer و Skrill."},
    {q:"ما الرسوم المطبقة؟", a:"السعر المعروض شامل جميع الرسوم، لا توجد رسوم خفية."},
  ];
  return (
      ...
      {faqs.map((f,i)=>(
        <div key={i}>
          <div className="faq-row" onClick={()=>setOpen(open===i?null:i)}
            style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px",cursor:"pointer",background:"transparent",borderBottom:i<faqs.length-1||open===i?`1px solid ${C.borderAlt}`:"none"}}>
...
function SuccessModal({platform,email,currency,amount,egp,onClose}: any){
  return(
    <div className="modal-overlay" onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:24,width:"100%",maxWidth:380,animation:"slideDown .35s ease both"}}>
        <div style={{textAlign:"center",marginBottom:18}}>
          ...
          <div style={{fontSize:12,color:C.sub,marginTop:6,lineHeight:1.8}}>سيتم تنفيذ طلبك خلال دقائق بعد المراجعة</div>
        </div>
        <div style={{background:C.cardAlt,borderRadius:12,padding:"14px",marginBottom:16}}>
          {[["المنصة",platform,C.text],["البريد",email,"#4F8EF7"],["العملة",currency,C.sub],["المبلغ المرسل",`${amount} ${currency}`,C.yellow],["المخصوم",`£ ${egp}`,C.red]].map(([l,v,c],i,a)=>(
...
export default function PayPalPage({balance=5000, setBalance, onBack, onAddToCart}: any){
  ...
  const egp=+(num*(currency==="USD"?RATE:currency==="EUR"?RATE*1.08:RATE*1.27)).toFixed(2);
  const canBuy=balance>=egp&&egp>0&&email.trim();
  ...
  const handleAddToCartInternal = () => {
    ...
    if(num<=0){setError("يرجى إدخال مبلغ صحيح");return;}
    if(onAddToCart) {
      onAddToCart({ name: platform, icon: "💳", total: egp, fields: { "البريد": email, "المبلغ": `${num} ${currency}` } });
      setShowAdded(true);
    }
  };
  ...
  return(
    <>
      <style>{css}</style>
      {showModal&&<SuccessModal platform={platform} email={email} currency={currency} amount={num} egp={egp} onClose={()=>setShowModal(false)}/>}
      <div dir="rtl" style={{fontFamily:"'Cairo',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",paddingBottom:40}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 16px 16px"}}>
          ...
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:16,fontWeight:900,color:C.text}}>PayPal & Payeer & Skrill</div>
            <div style={{fontSize:12,fontWeight:700,color:C.sub}}>تحويل فوري</div>
          </div>
        ...
        <div style={{padding:"0 14px",display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 16px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:12,color:C.sub,display:"flex",alignItems:"center",gap:5}}><span style={{color:C.yellow}}>⚡</span><span>تنفيذ سريع</span></div>
              </div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.borderAlt}`,borderRadius:14,padding:"13px 16px"}}>
            <p style={{fontSize:13,color:C.sub,lineHeight:1.9}}>أرسل رصيداً لأي حساب PayPal أو Payeer أو Skrill باستخدام رصيد محفظتك مباشرةً.</p>
          </div>

          {/* PLATFORM SELECT */}
          <div className="fadeUp">
            <label style={{fontSize:12,color:C.sub,fontWeight:700,display:"block",marginBottom:8}}>اختر المنصة</label>
            <select className="inp" value={platform} onChange={e=>setPlatform(e.target.value)} style={selectStyle as any}>
          ...
          <div className="fadeUp" style={{animationDelay:".05s"}}>
            <label style={{fontSize:12,color:C.sub,fontWeight:700,display:"block",marginBottom:8}}>البريد الإلكتروني للحساب</label>
          ...
          <div className="fadeUp" style={{animationDelay:".08s"}}>
            ...
            <select className="inp" value={currency} onChange={e=>setCurrency(e.target.value)} style={selectStyle as any}>
          ...
          <div className="fadeUp" style={{animationDelay:".1s"}}>
            <label style={{fontSize:12,color:C.sub,fontWeight:700,display:"block",marginBottom:8}}>المبلغ ({currency})</label>
            <div style={{position:"relative"}}>
              <input className="inp" value={amount} onChange={e=>setAmount(e.target.value.replace(/[^0-9.]/g,""))} placeholder="مثال: 20" dir="ltr" style={{...inputStyle,paddingLeft:60} as any}/>
              <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:12,color:C.sub2,fontWeight:700,pointerEvents:"none"}}>{currency}</span>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
            <button className="btn" onClick={handleBuy} disabled={loading}
              ...
              {loading?<span style={{display:"inline-flex",alignItems:"center",gap:10}}><span style={{width:18,height:18,border:"2px solid rgba(0,0,0,.2)",borderTopColor:"#1a0f00",borderRadius:"50%",animation:"spin 1s linear infinite",display:"inline-block"}}/>جاري...</span>:"إرسال الآن ⚡"}
            </button>
